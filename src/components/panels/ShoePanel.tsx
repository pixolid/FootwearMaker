...
export function ShoePanel({ shoeFile, onFileChange, onOpenLibrary }: ShoePanelProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-semibold mb-1">Step 1: Load Shoe</h3>
        <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          Upload a shoe model or select from the library
        </p>
      </div>
      ...
      {shoeFile && (
        ...
        >
          Shoe model loaded successfully
        </div>
      )}
    </div>
  )
